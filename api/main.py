...
if response.status_code == 200:
    ...
    # Sélectionner tous les éléments <li> contenant les poèmes
    poem_elements = soup.select('ul li')
    ...
    for index, elem in enumerate(poem_elements):
        ...
        # Extraire le nombre de commentaires dans la balise <span>
        comments_elem = elem.find('span')
        if comments_elem:
            comments = comments_elem.get_text(strip=True)
        else:
            comments = "Commentaires non trouvés"
